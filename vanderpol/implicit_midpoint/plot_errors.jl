...
ref_sol = [parse(Float128, ref_sol_str[2]), parse(Float128, ref_sol_str[3])]

# load other solutions from files
sols = [readdlm(sol_file, '\t', '\n') for sol_file in sol_files]

# calculate errors of other solutions (ignoring the # of time steps column)
errors = [l2(sol[:, 2:3] .- ref_sol', 2) for sol in sols]

# calculate time deltas
dts = [1.0 ./ sol[:, 1] for sol in sols]

# plot errors
p = plot(dts, errors, label=reshape(sol_files, (1, :)), dpi=300)
