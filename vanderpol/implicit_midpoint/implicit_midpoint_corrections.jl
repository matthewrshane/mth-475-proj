...
s = ArgParseSettings(description = "example$(NBS)usage:$(NBS)julia$(NBS)$PROGRAM_FILE$(NBS)Float64$(NBS)Float32$(NBS)100000$(NBS)1.0$(NBS)1")
@add_arg_table s begin
    ...
    "alpha"
        ...
        required = true
    "num_corrections"
        help = "Number of full precision correction steps to take."
        arg_type = Int
        required = true
    "--time", "-t"
...
num_corrections correction steps using the provided input and
initial values.
"""
...
const num_corrections = Int(get(parsed_args, "num_corrections", 1))
