...
#=
    Usage:
    julia implicit_midpoint_corrections_old.jl FullType ReducedType num_steps alpha num_corrections

    Example:
    julia implicit_midpoint_corrections_old.jl Float64 Float32 100000 1.0 1
=#
...
function updateF!(F::Array{ReduFloat}, y::Array{ReduFloat}, u::Array{ReduFloat}, dt::ReduFloat, alpha::ReduFloat)::Array{ReduFloat}
    ...
    F[1] = u[1] + dt * (y[2]) - y[1]
    F[2] = u[2] + dt * (alpha * (1 - (y[1]^2)) * y[2] - y[1]) - y[2]

    # TODO: Figure out the math behind this.
    #       Why is F defined in this way? The parts in parentheses come from the definition of van der Pol and letting y_1' = y_2.
    #       Implicit midpoint method defines u[n+1] = u[n] + dt * F(y^(1))? As per "Performance Evaluation ..."
...
function updateJ!(J::Matrix{ReduFloat}, y::Array{ReduFloat}, dt::ReduFloat, alpha::ReduFloat)::Matrix{ReduFloat}
    ...
    J[1, 1] = ReduFloat(-1.0)
    J[1, 2] = dt
    J[2, 1] = dt * (alpha * y[1] * y[2] - ReduFloat(1.0))                   # TODO: My calculation doesn't line up with this
    # J[2, 1] = -dt * (2 * alpha * y[1] * y[2] + ReduFloat(1.0))
    J[2, 2] = dt * alpha * (ReduFloat(1.0) - (y[1]^2)) - ReduFloat(1.0)

    # TODO: Figure out the math behind this.
    #       Seems like J is defined by normal Jacobian matrix as J[1, 1] = partial f_1 / partial y_1, etc...
    #       My calculations agree for J[1, 1], J[1, 2] and J[2, 2], but not J[2, 1]
    #       My J[2, 1] = -dt * (2 * alpha * y[1] * y[2] + 1)
...
"""
    driver(F, J, u, num_steps, time_start, time_end, alpha, num_corrections)

Performs the main calculations over num_steps time steps, with
num_corrections correction steps using the provided input and initial values.
"""
function driver(F::Array{ReduFloat}, J::Matrix{ReduFloat}, u::Array{FullFloat}, num_steps::Integer, time_start::FullFloat, time_end::FullFloat, alpha::FullFloat, num_corrections::Integer)::Array{FullFloat}
    # calculate dt
    ...
    J!(J::Matrix{ReduFloat}, y::Array{ReduFloat}) = updateJ!(J, y, ReduFloat(0.5 * dt), ReduFloat(alpha))
    ...
    for i = 1:num_steps
        ...
        F!(F::Array{ReduFloat}, y::Array{ReduFloat}) = updateF!(F, y, redu_u, ReduFloat(0.5 * dt), ReduFloat(alpha))
        ...
        for k = 1:num_corrections
            full_y[1] = u[1] + (FullFloat(0.5) * dt) * (full_y[2])
            full_y[2] = u[2] + (FullFloat(0.5) * dt) * (alpha * (FullFloat(1.0) - (full_y[1]^2)) * full_y[2] - full_y[1])
        end
...
const num_corrections = parse(Int64, ARGS[5])
...
u = driver(F, J, init_u, num_steps, time_start, time_end, alpha, num_corrections);
